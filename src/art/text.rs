...
fn make_p_render() -> Packet {
    ...
    let mid_right_out = Vertex::new([1.0, 0.4]); //4
    let mid_right_in = Vertex::new([0.8, 0.6]); //5
    let mid_left_top = Vertex::new([0.2, 0.6]); //6
    let mid_left_bot = Vertex::new([0.2, 0.4]); //7
    let bot_left_left = Vertex::new([0.0, 0.0]); //8
    let bot_left_right = Vertex::new([0.2, 0.0]); //9

    let vertices = vec![top_left_out, top_left_in, top_right_out, top_right_in, mid_right_out, mid_right_in, mid_left_top, mid_left_bot, bot_left_left, bot_left_right];
...
fn make_q_render() -> Packet {
    let top_left_out = Vertex::new([0.0, 1.0]); //0
    let top_left_in = Vertex::new([0.2, 0.8]); //1
    let bot_left_out = Vertex::new([0.0, 0.1]); //2
    let bot_left_in = Vertex::new([0.2, 0.3]); //3
    let bot_right_out = Vertex::new([0.9, 0.1]); //4
    let bot_right_in = Vertex::new([0.7, 0.3]); //5
    let top_right_out = Vertex::new([0.9, 1.0]); //6
    let top_right_in = Vertex::new([0.7, 0.8]); //7
    let slash_top_left = Vertex::new([0.3, 0.5]); //8
    let slash_top_right = Vertex::new([0.5, 0.5]); //9
    let slash_bot_left = Vertex::new([0.7, 0.0]); //10
    let slash_bot_right = Vertex::new([0.9, 0.0]); //11

    let vertices = vec![top_left_out, top_left_in, bot_left_out, bot_left_in, bot_right_out, bot_right_in, top_right_out, top_right_in, slash_top_left, slash_top_right, slash_bot_left, slash_bot_right];
...
fn make_r_render() -> Packet {
    ...
    let bot_left_left = Vertex::new([0.0, 0.0]); //8
    let bot_left_right = Vertex::new([0.2, 0.0]); //9
    let bot_right_left = Vertex::new([0.8, 0.0]); //10
    let bot_right_right = Vertex::new([1.0, 0.0]); //11

    let vertices = vec![top_left_out, top_left_in, top_right_out, top_right_in, mid_right_out, mid_right_in, mid_left_top, mid_left_bot, bot_left_left, bot_left_right, bot_right_left, bot_right_right];
...
fn make_s_render() -> Packet {
    ...
    let top_left_out = Vertex::new([0.0, 1.0]); //2
    let top_left_in = Vertex::new([0.2, 0.8]); //3
    let mid_left_out = Vertex::new([0.0, 0.4]); //4
    let mid_left_in = Vertex::new([0.2, 0.6]); //5
    let mid_right_out = Vertex::new([1.0, 0.6]); //6
    let mid_right_in = Vertex::new([0.8, 0.4]); //7
    let bot_right_out = Vertex::new([1.0, 0.0]); //8
    let bot_right_in = Vertex::new([0.8, 0.2]); //9
    let bot_left_bot = Vertex::new([0.0, 0.0]); //10
    let bot_left_top = Vertex::new([0.0, 0.2]); //11

    let vertices = vec![top_right_top, top_right_bot, top_left_out, top_left_in, mid_left_out, mid_left_in, mid_right_out, mid_right_in, bot_right_out, bot_right_in, bot_left_bot, bot_left_top];
...
fn make_t_render() -> Packet {
    let top_left_top = Vertex::new([0.0, 1.0]); //0
    let top_left_bot = Vertex::new([0.0, 0.8]); //1
    let top_right_top = Vertex::new([1.0, 1.0]); //2
    let top_right_bot = Vertex::new([1.0, 0.8]); //3
    let mid_left = Vertex::new([0.4, 0.8]); //4
    let mid_right = Vertex::new([0.6, 0.8]); //5
    let bot_left = Vertex::new([0.4, 0.0]); //6
    let bot_right = Vertex::new([0.6, 0.0]); //7

    let vertices = vec![top_left_top, top_left_bot, top_right_top, top_right_bot, mid_left, mid_right, bot_left, bot_right];
...
fn make_u_render() -> Packet {
    ...
    let vertices = vec![top_left_left, top_left_right, bot_left_out, bot_left_in, bot_right_out, bot_right_in, top_right_left, top_right_right];
...
fn make_v_render() -> Packet {
    let top_left_left = Vertex::new([0.0, 1.0]); //0
    let top_left_right = Vertex::new([0.2, 1.0]); //1
    let bot_mid_left = Vertex::new([0.4, 0.0]); //2
    let bot_mid_top = Vertex::new([0.5, 0.2]); //3
    let bot_mid_right = Vertex::new([0.6, 0.0]); //4
    let top_right_left = Vertex::new([0.8, 1.0]); //5
    let top_right_right = Vertex::new([1.0, 1.0]); //6

    let vertices = vec![top_left_left, top_left_right, bot_mid_left, bot_mid_top, bot_mid_right, top_right_left, top_right_right];
...
fn make_w_render() -> Packet {
    let top_left_left = Vertex::new([0.0, 1.0]); //0
    let top_left_right = Vertex::new([0.2, 1.0]); //1
    let bot_left_left = Vertex::new([0.2, 0.0]); //2
    let bot_left_right = Vertex::new([0.4, 0.0]); //3
    let top_mid_left = Vertex::new([0.4, 1.0]); //4
    let top_mid_right = Vertex::new([0.6, 1.0]); //5
    let bot_right_left = Vertex::new([0.6, 0.0]); //6
    let bot_right_right = Vertex::new([0.8, 0.0]); //7
    let top_right_left = Vertex::new([0.8, 1.0]); //8
    let top_right_right = Vertex::new([1.0, 1.0]); //9

    let vertices = vec![top_left_left, top_left_right, bot_left_left, bot_left_right, top_mid_left, top_mid_right, bot_right_left, bot_right_right, top_right_left, top_right_right];
...
fn make_x_render() -> Packet {
    let top_left_left = Vertex::new([0.0, 1.0]); //0
    let top_left_right = Vertex::new([0.2, 1.0]); //1
    let bot_right_left = Vertex::new([0.8, 0.0]); //2
    let bot_right_right = Vertex::new([1.0, 0.0]); //3
    let top_right_left = Vertex::new([0.8, 1.0]); //4
    let top_right_right = Vertex::new([1.0, 1.0]); //5
    let bot_left_left = Vertex::new([0.0, 0.0]); //6
    let bot_left_right = Vertex::new([0.2, 0.0]); //7

    let vertices = vec![top_left_left, top_left_right, bot_right_left, bot_right_right, top_right_left, top_right_right, bot_left_left, bot_left_right];
...
fn make_y_render() -> Packet {
    let top_left_left = Vertex::new([0.0, 1.0]); //0
    let top_left_right = Vertex::new([0.2, 1.0]); //1
    let mid_mid_left = Vertex::new([0.4, 0.5]); //2
    let mid_mid_top = Vertex::new([0.5, 0.6]); //3
    let mid_mid_right = Vertex::new([0.6, 0.5]); //4
    let top_right_left = Vertex::new([0.8, 1.0]); //5
    let top_right_right = Vertex::new([1.0, 1.0]); //6
    let bot_mid_left = Vertex::new([0.4, 0.0]); //7
    let bot_mid_right = Vertex::new([0.6, 0.0]); //8

    let vertices = vec![top_left_left, top_left_right, mid_mid_left, mid_mid_top, mid_mid_right, top_right_left, top_right_right, bot_mid_left, bot_mid_right];
...
fn make_z_render() -> Packet {
    ...
    let top_left_bot = Vertex::new([0.0, 0.8]); //1
    let top_right_out = Vertex::new([1.0, 1.0]); //2
    let top_right_in = Vertex::new([0.65, 0.8]); //3
    let bot_left_out = Vertex::new([0.0, 0.0]); //4
    let bot_left_in = Vertex::new([0.35, 0.2]); //5
    let bot_right_top = Vertex::new([1.0, 0.2]); //6
    let bot_right_bot = Vertex::new([1.0, 0.0]); //7

    let vertices = vec![top_left_top, top_left_bot, top_right_out, top_right_in, bot_left_out, bot_left_in, bot_right_top, bot_right_bot];
...
fn make_1_render() -> Packet {
    let top_left_top = Vertex::new([0.0, 1.0]); //0
    let top_left_bot = Vertex::new([0.0, 0.8]); //1
    let top_right_out = Vertex::new([0.6, 1.0]); //2
    let top_right_in = Vertex::new([0.4, 0.8]); //3
    let bot_mid_left = Vertex::new([0.4, 0.0]); //4
    let bot_mid_right = Vertex::new([0.6, 0.0]); //5

    let vertices = vec![top_left_top, top_left_bot, top_right_out, top_right_in, bot_mid_left, bot_mid_right];
...
fn make_2_render() -> Packet {
    ...
    let mid_right_out = Vertex::new([1.0, 0.4]); //4
    let mid_right_in = Vertex::new([0.8, 0.6]); //5
    let mid_left_out = Vertex::new([0.0, 0.6]); //6
    let mid_left_in = Vertex::new([0.2, 0.4]); //7
    let bot_left_out = Vertex::new([0.0, 0.0]); //8
    let bot_left_in = Vertex::new([0.2, 0.2]); //9
    let bot_right_top = Vertex::new([1.0, 0.2]); //10
    let bot_right_bot = Vertex::new([1.0, 0.0]); //11

    let vertices = vec![top_left_top, top_left_bot, top_right_out, top_right_in, mid_right_out, mid_right_in, mid_left_out, mid_left_in, bot_left_out, bot_left_in, bot_right_top, bot_right_bot];
...
fn make_3_render() -> Packet {
    ...
    let mid_left_top = Vertex::new([0.0, 0.6]); //6
    let mid_left_bot = Vertex::new([0.0, 0.4]); //7
    let bot_right_out = Vertex::new([1.0, 0.0]); //8
    let bot_right_in = Vertex::new([0.8, 0.2]); //9
    let bot_left_top = Vertex::new([0.0, 0.2]); //10
    let bot_left_bot = Vertex::new([0.0, 0.0]); //11

    let vertices = vec![top_left_top, top_left_bot, top_right_out, top_right_in, mid_right_top, mid_right_bot, mid_left_top, mid_left_bot, bot_right_out, bot_right_in, bot_left_top, bot_left_bot];
...
fn make_4_render() -> Packet {
    let top_left_left = Vertex::new([0.0, 1.0]); //0
    let top_left_right = Vertex::new([0.2, 1.0]); //1
    let mid_left_out = Vertex::new([0.0, 0.4]); //2
    let mid_left_in = Vertex::new([0.2, 0.6]); //3
    let mid_right_top = Vertex::new([0.8, 0.6]); //4
    let mid_right_bot = Vertex::new([0.8, 0.4]); //5
    let top_right_left = Vertex::new([0.8, 1.0]); //6
    let top_right_right = Vertex::new([1.0, 1.0]); //7
    ...
    let vertices = vec![top_left_left, top_left_right, mid_left_out, mid_left_in, mid_right_top, mid_right_bot, top_right_left, top_right_right, bot_right_left, bot_right_right];
...
fn make_5_render() -> Packet {
    ...
    let vertices = vec![top_right_top, top_right_bot, top_left_out, top_left_in, mid_left_out, mid_left_in, mid_right_out, mid_right_in, bot_right_out, bot_right_in, bot_left_top, bot_left_bot];
...
fn make_6_render() -> Packet {
    ...
    let mid_right_out = Vertex::new([1.0, 0.6]); //8
    let mid_right_in = Vertex::new([0.8, 0.4]); //9
    let mid_left_top = Vertex::new([0.2, 0.6]); //10
    let mid_left_bot = Vertex::new([0.2, 0.4]); //11

    let vertices = vec![top_right_top, top_right_bot, top_left_out, top_left_in, bot_left_out, bot_left_in, bot_right_out, bot_right_in, mid_right_out, mid_right_in, mid_left_top, mid_left_bot];
...
fn make_7_render() -> Packet {
    let top_left_top_left = Vertex::new([0.0, 1.0]); //0
    let top_left_bot_left = Vertex::new([0.0, 0.8]); //1
    let top_left_bot_right = Vertex::new([0.2, 0.8]); //2
    let top_left_top_right = Vertex::new([0.2, 1.0]); //3
    let top_right_out = Vertex::new([1.0, 1.0]); //4
    let top_right_in = Vertex::new([0.8, 0.8]); //5
    let bot_right_left = Vertex::new([0.8, 0.0]); //6
    let bot_right_right = Vertex::new([1.0, 0.0]); //7

    let vertices = vec![top_left_top_left, top_left_bot_left, top_left_bot_right, top_left_top_right, top_right_out, top_right_in, bot_right_left, bot_right_right];
